...
class ModelArguments:
    ...
    pretrained_dir: str = field(
        default=None,
        metadata={"help": "The path of the model."},
    )
...
def main():
    ...
    # Setup logging
    # Modified:
    if model_args.custom_model == "hierarchical":
        inter_path = path_adder(pretrained_args, finetuning=True, custom_model=model_args.custom_model, c_args=model_args)
    elif model_args.custom_model == "longformer":
        inter_path = path_adder(model_args, finetuning=True, custom_model=model_args.custom_model)
    else:
        inter_path = path_adder(model_args, finetuning=True)
    inter_path += datetime.now().strftime("%Y_%m_%d-%H_%M_%S")
    training_args.output_dir = os.path.join(training_args.output_dir, inter_path)
    ...
    if model_args.custom_model == "longformer":
        psm = pretrained_sequence_model_selector(select_base(model_args.pretrained_dir))
        model = get_attention_injected_model(psm)
        model = model.from_pretrained(
            model_args.pretrained_dir,  # /checkpoint-14500
            max_length=data_args.max_seq_length,
            num_labels=num_labels
        )
    elif model_args.custom_model is None:
        config = AutoConfig.from_pretrained(model_args.pretrained_dir, num_labels=num_labels)
        model = AutoModelForSequenceClassification.from_pretrained(
            model_args.pretrained_dir,
            config=config,
        )
    ...
    if model_args.custom_model == "longformer":
        ...
        tokenizer = AutoTokenizer.from_pretrained(model_args.pretrained_dir)
    ...
    def data_collator(examples):
        ...
        tokenized_examples = tokenizer(*[questions, passages], padding=True, return_tensors="pt",
